...
export default function WeekNav({
  ...
  return (
    <div className="flex items-center gap-4">
      <Link href={`${basePath}?week=${prev}`} aria-label="Previous week" className="rounded-full border-2 border-buttercream px-3 py-1">
        ‹
      </Link>
      <span className="font-bold">{formatWeekRange(monday)}</span>
      <Link href={`${basePath}?week=${next}`} aria-label="Next week" className="rounded-full border-2 border-buttercream px-3 py-1">
        ›
      </Link>
    </div>
  );
